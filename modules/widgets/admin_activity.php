...
<tr bgcolor="#efefef" style="text-align:center;font-weight:bold;"><td>'.$_ADMINLANG['fields']['admin'].'</td><td>'.$_ADMINLANG['fields']['ipaddress'].'</td><td>'.$_ADMINLANG['system']['lastaccess'].'</td></tr>';
    ...
    while ($data = mysql_fetch_array($result)) {
        $content .= '<tr bgcolor="#ffffff" style="text-align:center;"><td>'.$data["adminusername"].'</a></td><td><a href="http://nullrefer.com/?http://www.geoiptool.com/en/?IP='.$data["ipaddress"].'" target="_blank">'.$data["ipaddress"].'</a></td><td>'.fromMySQLDate($data["lastvisit"],true).'</td></tr>';
    }
    $content .= '</table>';
