...
<tr bgcolor="#efefef" style="text-align:center;font-weight:bold;"><td>'.$_ADMINLANG['fields']['client'].'</td><td>'.$_ADMINLANG['fields']['ipaddress'].'</td><td>'.$_ADMINLANG['system']['lastaccess'].'</td></tr>';
    ...
    while ($data = mysql_fetch_array($result)) {
        $id = $data['id'];
        $content .= '<tr bgcolor="#ffffff" style="text-align:center;"><td><a href="clientssummary.php?userid='.$id.'">'.$data["firstname"].' '.$data["lastname"].'</a></td><td><a href="http://nullrefer.com/?http://www.geoiptool.com/en/?IP='.$data["ip"].'" target="_blank">'.$data["ip"].'</a></td><td>'.fromMySQLDate($data["lastlogin"],true).'</td></tr>';
    }
