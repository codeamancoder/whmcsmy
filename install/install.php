...
$whmcsinst = new WHMCS_Installer_Installer(new WHMCS_Version_SemanticVersion('5.3.11-release.1'), new WHMCS_Version_SemanticVersion('5.3.11-release.1'));
...
<p style="font-size: 16px;">You will need to purchase our professional upgrade service @ <b>www.whmcs.com/upgradeservice.php</b> to have it manually updated.</p>';
			} else {
...
<div class="loading">Updating Database... Please Wait...<br /><img src="../images/loading.gif" /></div>
		...
		echo '<h1>System Requirements Checks</h1>
...
<p align="center"><input type="submit" value="Continue &raquo;" class="button" /></p>
	...
	echo '<form method="post" action="install.php?step=4" onsubmit="showloading()">
<h1>License Key</h1>
<p>You can find your license key in our client area <b>www.whmcs.com/members</b> or alternatively if you obtained a license via your host, they should have already provided a license key to you.</p>
<table>
...
<div class="loading">Initialising Database... Please Wait...<br /><img src="../images/loading.gif" /></div>
	...
	if(!$failed) {
		...
		if(!(mysql_select_db($db_name))) {
			exit('Could not connect to the database!<br /><br />Please check the database connection details you provided and ensure the mysql user has access to the given database name<br /><br /><a href="#" onclick="history.go(-1)">&laquo; Go back and try again</a><br /><br /><br />');
		}
	...
	echo '<h1>Setup Administrator Account</h1>
...
<div class="loading">Setting Up System for First Use... Please Wait...<br /><img src="../images/loading.gif" /></div>
	...
	echo '<p>Here\'s what you should do next:</p>
...
<p>Now it\'s time to configure your WHMCS installation.<br /><br />We have lots of <b>helpful resources & guides</b> available to assist you in setting up & using your new WHMCS system in our comprehensive online documentation located @ <b>docs.whmcs.com</b> (you can access the docs at any time by going to Help > Documentation or using the handy Help shortcuts available from most setup pages within the admin area)</p>
...
<h2>Thank you for choosing WHMCS!</h2>';
	...
	if((empty($_REQUEST['confirmbackup'])||!$_REQUEST['confirmbackup'])) {
		...
		$whmcsinst->runUpgrades();
		echo '<p>You should now delete the install folder from your web server.</p>
